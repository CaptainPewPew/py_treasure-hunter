...
def display_board(board,show_treasure = False):
    '''takes in a list with sublist to display game board, if show_treasure = 
    True 'T' are displayed, otherwise 'T' is replaced with ' ', does not return 
    any value'''
    mask = ' '
    if show_treasure:
        mask = 'T'
...
def make_user_move(board):
    ...
    are between 0 and 4. Returns true(places '$') if found treasure & vise versa
    (places 'X')''' 
    move = True
    while move:
        try :
            ...
            if row <= 4 and row >= 0 and column <= 4 and column >= 0:
                #checks if board index have already been checked
